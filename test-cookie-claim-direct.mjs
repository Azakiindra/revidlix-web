// Test direct flow to z2 (no worker) with cookies kept
...
async function main() {
  ...
  const r1 = await request(`${Z2}/api/movies/${SLUG}`);
  ...
  const r2 = await request(`${Z2}/api/views/track`, 'POST', {
    contentType: 'movie',
    contentId: uuid,
  });
  ...
  const r3 = await request(`${Z2}/api/watch/play-info/movie/${uuid}`);
  ...
  console.log('\n[4] Claiming session...');
  const r4 = await request(`${Z2}/api/watch/session/claim`, 'POST', {
